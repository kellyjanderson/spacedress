...
[`IMPLEMENTATION_PLAN.md`](IMPLEMENTATION_PLAN.md) is the executable TDD backlog. Pair/instance styling added after its IDs were numbered is tracked in [`docs/instance-pair-styling-tdd.md`](docs/instance-pair-styling-tdd.md). Capability-gated native split-layout restoration is tracked in [`docs/split-layout-control-tdd.md`](docs/split-layout-control-tdd.md).

When a prompt names an `SD-###`, `SI-###`, or `SL-###` slice:
...
- A split-pair rule may remember a preferred member-relative allocation such as Chrome 65% / iTerm2 35%; current left/right position remains runtime geometry.
- Split-layout restoration is capability-gated and apply-once by default. Do not continuously fight manual divider changes.
...
- Do not silently escalate split-layout restoration from supported Accessibility/system APIs to private window mutation.
...
- DSAS is application-facing and must not absorb SpaceDress branding, targeting rules, pair selectors, instance leases, split-layout preferences, or the internal multi-app user manifest.
...
- for layout mutation, test whether AX attributes are actually settable and verify achieved geometry after every attempt;
- treat app/system minimum-size clamping as an observed result, not a reason for an enforcement loop;
...
Pair layout preferences are separate user/workspace behavior. They do not enter DSAS or visual appearance resolution.
